...
# -*- coding: utf-8 -*-
"""
Instalador Automático de Extensión pyRevit - IaEnRvt
===================================================

Este instalador instala la extensión del bot NLP de manera automática y garantiza
que el botón aparezca en Revit correctamente.
...
class InstaladorExtensionPyRevit:
    def __init__(self):
    ...
    def instalar_extension(self):
        """Instalar la extensión copiando archivos"""
        try:
            ...
            if os.path.exists(self.bot_origen):
                shutil.copy2(self.bot_origen, bot_destino)
                print(f"✅ Bot NLP copiado a: {bot_destino}")
            
            # Crear enlace simbólico si es posible (para desarrollo)
            try:
                if self.sistema != "Windows":
                    link_path = os.path.join(extensiones_dir, "IaEnRvt.extension")
                    if os.path.exists(link_path):
                        os.remove(link_path)
                    os.symlink(self.extension_origen, link_path)
                    print("✅ Enlace simbólico creado para desarrollo")
            except Exception as e:
    ...
    def mostrar_instrucciones(self):
        ...
        print("• Si no aparece la extensión, reinicia Revit")
    ...
    def ejecutar(self):
        ...
        print("🚀 INSTALADOR AUTOMÁTICO DE EXTENSIÓN PYREVIT - IaEnRvt")
        print("="*60)
...
def main():
    """Función principal"""
    try:
        instalador = InstaladorExtensionPyRevit()
